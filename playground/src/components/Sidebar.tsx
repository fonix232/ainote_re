...
import type { KnownDevice as KnownDeviceEntry } from '@ainote/protocols';
import { store } from '../store/index.js';
import { PROTOCOLS } from '../protocols/registry.js';
import { activeProto } from '../protocols/registry.js';
import type { AnyDebugCommand } from '@ainote/protocols';
...
export function LeftPanel({ onConnectKnown, onForgetKnown }: SidebarProps) {
  ...
  const cmds = proto?.debugCommands ?? [];
  ...
  return (
    <aside class={`${collapsed.value ? 'w-8' : 'w-56'} shrink-0 flex flex-col overflow-y-auto bg-base-200 border-r border-base-content/10 transition-[width] duration-200`}>
      ...
      {!collapsed.value && <div class="flex flex-col gap-3 p-3 flex-1">
        ...
        {/* Commands */}
        {isConn && cmds.length > 0 && (
          <div>
            <div class="divider divider-start text-xs text-base-content/30 my-0">Commands</div>
            <div class="flex flex-col gap-1.5 mt-2">
              {cmds.map(cmd => <CmdCard key={cmd.label} cmd={cmd} />)}
            </div>
          </div>
        )}
...
function CmdCard({ cmd }: { cmd: AnyDebugCommand }) {
  if ('kind' in cmd && cmd.kind === 'toggle') return <CmdToggle cmd={cmd} />;
  if ('kind' in cmd && cmd.kind === 'select') return <CmdSelect cmd={cmd} />;
  return <CmdButton cmd={cmd} />;
}

function CmdButton({ cmd }: { cmd: import('@ainote/protocols').DebugCommand }) {
  ...
  return (
    <button class="btn btn-xs btn-outline w-full" onClick={onClick}>{cmd.label}</button>
  );
}
